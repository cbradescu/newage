define(function(require) {
    ...
    var GuestNotifierView = require('cbscheduler/js/app/views/guest-notifier-view');
    ...
    GuestsPlugin = BasePlugin.extend({
        ...
        hasParentEvent: function(eventModel) {
            ...
            var alias = eventModel.get('schedulerAlias');
            if (parentEventId) {
                result = Boolean(this.main.getConnectionCollection().find(function(c) {
                    return c.get('schedulerAlias') === alias &&
                        this.collection.get(c.get('schedulerUid') + '_' + parentEventId);
                }, this));
            }
            return result;
        },
        ...
        onEventDeleted: function(eventModel) {
            ...
            if (this.hasLoadedGuestEvents(eventModel)) {
                ...
                guestEventModels = this.findGuestEventModels(eventModel);
                for (i = 0; i < guestEventModels.length; i++) {
                    this.main.getSchedulerElement().fullCalendar('removeEvents', guestEventModels[i].id);
