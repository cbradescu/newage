...
class SchedulerPropertyProvider
{
    const CALENDAR_PROPERTY_CLASS = 'CB\Bundle\SchedulerBundle\Entity\SchedulerProperty';
     ...
     * @return array
     */
    public function getItems($schedulerId)
    {
        $selectItems = [];
        $intCasts    = [];
        ...
        $qb   = $repo->createQueryBuilder('o')
            ...
            ->where('o.targetScheduler = :scheduler_id')
            ->setParameter('scheduler_id', $schedulerId)
            ->orderBy('o.id');
     ...
     *
     * @return array of [schedulerAlias, scheduler, visible]
     */
    public function getItemsVisibility($schedulerId, $subordinate)
    {
        $qb = $this->doctrineHelper
            ->getEntityRepository(self::CALENDAR_PROPERTY_CLASS)
            ->createQueryBuilder('o')
            ->select('o.schedulerAlias, o.scheduler, o.visible')
            ->where('o.targetScheduler = :scheduler_id')
            ->setParameter('scheduler_id', $schedulerId);
        if (!$subordinate) {
            $qb
                ->andWhere('o.schedulerAlias = :alias AND o.scheduler = :scheduler_id')
                ->setParameter('alias', Scheduler::CALENDAR_ALIAS);
        }
